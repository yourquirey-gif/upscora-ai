...
export default function LoginPage(){
 ...
 return <main className="min-h-screen bg-[#07090d] px-6 py-8 text-white">
  ...
  <div className="mx-auto grid min-h-[calc(100vh-90px)] place-items-center py-12"><div className="w-full max-w-md rounded-[30px] border border-white/10 bg-white/[0.035] p-7 shadow-2xl shadow-black/30 sm:p-9">
   <div className="mb-8"><div className="mb-5 flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/5"><Sparkles size={18}/></div><h1 className="text-3xl font-semibold tracking-tight">Welcome back.</h1><p className="mt-2 text-sm leading-6 text-white/45">Continue your UPSC preparation with your personalized workspace.</p></div>
   <form onSubmit={submit} className="space-y-4"><label className="block"><span className="mb-2 block text-xs text-white/50">Email</span><div className="flex items-center rounded-xl border border-white/10 bg-black/20 px-3"><Mail size={16} className="text-white/35"/><input required value={email} onChange={e=>setEmail(e.target.value)} type="email" placeholder="you@example.com" className="w-full bg-transparent px-3 py-3 text-sm outline-none placeholder:text-white/25"/></div></label><label className="block"><span className="mb-2 block text-xs text-white/50">Password</span><div className="flex items-center rounded-xl border border-white/10 bg-black/20 px-3"><LockKeyhole size={16} className="text-white/35"/><input required type="password" placeholder="••••••••" className="w-full bg-transparent px-3 py-3 text-sm outline-none placeholder:text-white/25"/></div></label><button className="w-full rounded-xl bg-white py-3.5 text-sm font-semibold text-black">{loading?"Signing in…":"Sign in"}</button></form>
   <p className="mt-6 text-center text-xs text-white/35">New to UPSCora? <Link href="/signup" className="text-white hover:underline">Create an account</Link></p>
  </div></div>
 </main>
}